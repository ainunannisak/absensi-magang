...
class Kehadiran_model extends CI_Model
{
    ...
    public function getRekap()
    {
        $query = "SELECT DISTINCT presents.user_id, user.name AS name, positions.position_name AS position, 
				(SELECT COUNT(information) FROM presents WHERE (information = 'M') AND (STATUS = 1) AND (presents.user_id = user.id_users)) AS M,
				(SELECT COUNT(information) FROM presents WHERE (information = 'I') AND (STATUS = 1) AND (presents.user_id = user.id_users)) AS I,
				(SELECT COUNT(information) FROM presents WHERE (information = 'S') AND (STATUS = 1) AND (presents.user_id = user.id_users)) AS S,
				(SELECT COUNT(information) FROM presents WHERE (STATUS = 1) AND (presents.user_id = user.id_users)) AS total
			FROM presents JOIN user
			ON presents.user_id = user.id_users
